...
exports.createFee = async (req, res) => {
  try {
    ...
    if (online_amount > total_amount) {
      return res.status(400).json({ error: 'Online amount cannot exceed total amount' });
    }
    const cash_amount = (Number(total_amount) - Number(online_amount)).toFixed(2);
...
exports.listByDate = async (req, res) => {
  try {
    ...
    const rows = await Fee.findAll({ where: { date: targetDate }, order: [['createdAt', 'DESC']] });
    res.json(rows);
  } catch (e) {
